...
// Multipliers Test Script – Likert scale version
...
function calculateResults(answers) {
    ...
    // Count strengths and weaknesses using Likert scale
    for (let i = 0; i < questions.length; i++) {
        const q = questions[i];
        const competency = q.competency;
        const invert = q.invert;
        const answer = answers[i]; // value from 1 to 5

        if (!(competency in results)) continue;

        if ((answer >= 4 && !invert) || (answer <= 2 && invert)) {
            results[competency]["strength"] += 1;
        } else if ((answer <= 2 && !invert) || (answer >= 4 && invert)) {
